-- =====================================================
-- Event Hub Connect - Database Schema
-- PostgreSQL (Supabase)
-- =====================================================
...
-- =====================================================
-- 4. PACKAGES TABLE (Separate Entity)
-- =====================================================

-- Packages
-- package.listing_category_id = Package's category (can differ from vendor.category)
CREATE TABLE IF NOT EXISTS packages (
    ...
    listing_category_id VARCHAR(50) NOT NULL REFERENCES categories(id), -- Package's category
...
-- Package Event Types (Junction Table)
-- Many-to-Many: Packages ↔ Event Types
CREATE TABLE IF NOT EXISTS package_event_types (
    package_id UUID REFERENCES packages(id) ON DELETE CASCADE,
    event_type_id INTEGER REFERENCES event_types(id) ON DELETE CASCADE,
    PRIMARY KEY (package_id, event_type_id),
    created_at TIMESTAMP DEFAULT NOW()
);
...
-- =====================================================
-- 5. INDIVIDUAL LISTINGS TABLE (Separate Entity)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS listings (
    ...
    unit VARCHAR(50), -- e.g., "per piece", "per set", "per hour"
...
-- Cart Items (can be package OR listing)
CREATE TABLE IF NOT EXISTS cart_items (
    ...
    package_id UUID REFERENCES packages(id) ON DELETE CASCADE,
    listing_id UUID REFERENCES listings(id) ON DELETE CASCADE,
    item_type VARCHAR(20) NOT NULL CHECK (item_type IN ('package', 'listing')),
    ...
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
    -- Ensure either package_id or listing_id is set, but not both
    CONSTRAINT cart_item_type_check CHECK (
        (item_type = 'package' AND package_id IS NOT NULL AND listing_id IS NULL) OR
        (item_type = 'listing' AND listing_id IS NOT NULL AND package_id IS NULL)
    )
);
...
-- Orders (can be package OR listing)
...
CREATE INDEX idx_orders_package ON orders(package_id) WHERE package_id IS NOT NULL;
CREATE INDEX idx_orders_listing ON orders(listing_id) WHERE listing_id IS NOT NULL;
